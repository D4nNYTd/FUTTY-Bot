    ...
    if (subcommandGroup === 'ticket') {
      ...
      if (ticketSub === 'role') {
        ...
        if (action === 'list') {
          ...
          return interaction.reply({ content: roles.length ? `Ticket staff roles: ${roles.map((id) => `<@&${id}>`).join(', ')}` : 'No ticket staff roles configured.', flags: MessageFlags.Ephemeral });
        }
        const role = interaction.options.getRole('role');
        ...
        if (action === 'add') {
          ...
          return interaction.reply({ content: `Added ${role} as ticket staff role.`, flags: MessageFlags.Ephemeral });
        }
        ticketRoles.remove(interaction.guildId, role.id);
        return interaction.reply({ content: `Removed ${role} from ticket staff roles.`, flags: MessageFlags.Ephemeral });
      }
